...
public class ProjectClassService : IProjectClassService
{
    ...
    public async Task Register(RegisterClassRequestDto requestDto)
    {
        var projectClass = await _projectClassRepository.GetByNameAsync(requestDto.Name);
        if (projectClass is not null) throw new CustomException(HttpStatusCode.BadRequest, $"A classe {requestDto.Name} já existe");

        var newProjectClass = ProjectClassMapper.ToEntity(requestDto);
        try
        {
